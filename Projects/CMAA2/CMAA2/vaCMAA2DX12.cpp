...
namespace VertexAsylum
{
    ...
    struct vaInputResourceHelperDX12 : InputResourceHelperDX12
    {
        ...
        vaInputResourceHelperDX12( vaRenderDeviceContextDX12 & context, const shared_ptr<vaTexture> & texture, D3D12_RESOURCE_STATES beforeState, D3D12_RESOURCE_STATES afterState ) 
            : Context(context), Texture(texture), InputResourceHelperDX12( (texture!=nullptr)?(AsDX12(*texture).GetResource()):(nullptr) ) 
        { 
    ...
    private:
        vaDrawResultFlags               Execute( vaRenderDeviceContext & deviceContext, ID3D12GraphicsCommandList* commandList, const InputResourceHelperDX12 & inOutColor, const InputResourceHelperDX12 & optionalInLuma, const InputResourceHelperDX12 & inColorMS, const InputResourceHelperDX12 & inColorMSComplexityMask );
...
vaCMAA2DX12::~vaCMAA2DX12( )
{
    ...
    AsDX12( GetRenderDevice() ).SafeReleaseAfterCurrentGPUFrameDone( m_rootSignature );
...
static void UpdatePSOIfNeeded( vaRenderDevice & device, ID3D12RootSignature * rootSignature, bool & allOk, const shared_ptr<vaComputeShader> & shader, int64 & prevShaderUniqueContentsID, ComPtr<ID3D12PipelineState> & pso )
{
    ...
    ComPtr<ID3DBlob> shaderBlob; int64 shaderUniqueContentsID; 
...
vaDrawResultFlags vaCMAA2DX12::Execute( vaRenderDeviceContext & deviceContext, ID3D12GraphicsCommandList* commandList, const InputResourceHelperDX12 & inOutColor, const InputResourceHelperDX12 & optionalInLuma, const InputResourceHelperDX12 & inColorMS, const InputResourceHelperDX12 & inColorMSComplexityMask )
{
    ...
    {
        // if not reading from inLumaReadonly then we'll be reading from color so we have to be in D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE
        if( m_inLumaReadonlySRV.IsNull() && inOutColor.BeforeState != D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE )
        ...
        // if reading from inLumaReadonly so not reading from color here so we only have to have color in D3D12_RESOURCE_STATE_UNORDERED_ACCESS for later
        if( !m_inLumaReadonlySRV.IsNull() && inOutColor.BeforeState != D3D12_RESOURCE_STATE_UNORDERED_ACCESS )
            commandList->ResourceBarrier(1, &CD3DX12_RESOURCE_BARRIER::Transition( inOutColor.Source, inOutColor.BeforeState, D3D12_RESOURCE_STATE_UNORDERED_ACCESS ) );
    ...
    {
        m_firstRun = false;
        VA_SCOPE_CPUGPU_TIMER( ClearControlBuffer, deviceContext );
    ...
    // Set up for the first DispatchIndirect
    {
        ...
        D3D12_RESOURCE_BARRIER barriers[2] = { 
            CD3DX12_RESOURCE_BARRIER::Transition( g_workingExecuteIndirectBufferResource.Get(), D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_INDIRECT_ARGUMENT ), 
            CD3DX12_RESOURCE_BARRIER::UAV( m_workingControlBufferResource.Get() ) }; // Ensure the actual item count loaded from m_workingControlBufferResource is correct
    ...
    // Process shape candidates DispatchIndirect
    {
        ...
        D3D12_RESOURCE_BARRIER barriers[2] = { 
            CD3DX12_RESOURCE_BARRIER::Transition( g_workingExecuteIndirectBufferResource.Get(), D3D12_RESOURCE_STATE_INDIRECT_ARGUMENT, D3D12_RESOURCE_STATE_UNORDERED_ACCESS ), 
            CD3DX12_RESOURCE_BARRIER::UAV( nullptr ) }; // Same as before the previous ComputeDispatchArgs - saves us from doing bunch of other barriers later too.
        commandList->ResourceBarrier( _countof(barriers), barriers );
    }
    ...
    if( m_textureSampleCount == 1 && m_inLumaReadonlySRV.IsNull() )
...
vaDrawResultFlags vaCMAA2DX12::DrawMS( vaRenderDeviceContext & deviceContext, const shared_ptr<vaTexture> & inoutColor, const shared_ptr<vaTexture> & inColorMS, const shared_ptr<vaTexture> & inColorMSComplexityMask )
{
    ...
    {
        vaInputResourceHelperDX12 rhIOColor( AsDX12(deviceContext), inoutColor, D3D12_RESOURCE_STATE_UNORDERED_ACCESS, D3D12_RESOURCE_STATE_UNORDERED_ACCESS );
        vaInputResourceHelperDX12 rhOILuma( AsDX12(deviceContext), nullptr, D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COMMON );
        vaInputResourceHelperDX12 rhIColorMS( AsDX12(deviceContext), inColorMS, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE );
        vaInputResourceHelperDX12 rhIColorComplexityMask( AsDX12( deviceContext ), inColorMSComplexityMask, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE );
        ...
        renderResults = Execute( deviceContext, AsDX12( deviceContext ).GetCommandList().Get(), rhIOColor, rhOILuma, rhIColorMS, rhIColorMS );
    }
    AsDX12( deviceContext ).BindDefaultStates(); // Re-bind descriptor heaps, root signatures, viewports, scissor rects and render targets if any
...
// These two Draw/DrawMS function should contain all framework-specific "glue" required to run CMAA2 DX12; everything else is mostly DX12 code
// (except shader compilation and the safely freeing of DX12 objects).
vaDrawResultFlags vaCMAA2DX12::Draw( vaRenderDeviceContext & deviceContext, const shared_ptr<vaTexture> & inoutColor, const shared_ptr<vaTexture> & optionalInLuma )
{
    // Track the external inputs so we can re-create if they change - even if formats and sizes are the same we'll have to update view descriptors.
    // This one is a bit tricky: just by looking at whether input texture ID3D12Resource ptr and/or ID3D12Resource::GetDesc changed we cannot determine for 
    // certain that the texture was not re-created (as it could get the same ptr), which would invalidate all our view descriptors looking into it. So we 
    ...
    vaDrawResultFlags renderResults;
    {
        vaInputResourceHelperDX12 rhIOColor( AsDX12(deviceContext), inoutColor, (optionalInLuma!=nullptr)?(D3D12_RESOURCE_STATE_UNORDERED_ACCESS):(D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE), D3D12_RESOURCE_STATE_UNORDERED_ACCESS );
        vaInputResourceHelperDX12 rhOILuma( AsDX12(deviceContext), optionalInLuma, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE, D3D12_RESOURCE_STATE_NON_PIXEL_SHADER_RESOURCE );
        vaInputResourceHelperDX12 rhIColorMS( AsDX12(deviceContext), nullptr, D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COMMON );
        vaInputResourceHelperDX12 rhIColorComplexityMask( AsDX12( deviceContext ), nullptr, D3D12_RESOURCE_STATE_COMMON, D3D12_RESOURCE_STATE_COMMON );
